...
All display fields (`name`, `state`, `photoUrl`, `phone`, `url`) move to sessionStorage.
Note: `district` is **not** on the `Representative` type in `lib/types.ts` and is therefore not available in sessionStorage; see Section 2 for how RepHeader handles this.
...
Next.js `<Link onClick>` fires before navigation, so the write is always complete before the new page loads. This link is already inside the `!isSenator` branch, so senators are excluded correctly.
...
- Update the file header comment to reflect that display data now comes from sessionStorage, not URL params
...
- `state` → meta line (state only; `district` is not on the `Representative` type and will not be shown)
- `photoUrl` → photo (see below)
- `phone` → tel link with phone icon
- `url` → website link
- `party`, `chamber` → already shown from props; consistent since sessionStorage has the same values
...
### 4b. `lib/voteprint/types.ts` and `lib/voteprint/voteMappings.ts`

**`lib/voteprint/types.ts`** — add one field to the `Vote` interface:
...
**`lib/voteprint/voteMappings.ts`** — add `stance` to the private `VoteMappingEntry` interface (which lives in this file, not in `types.ts`):
...
`enrichWithCategory` accepts `position: Vote['position']` as a second parameter — this is the **already-normalized** `'yea' | 'nay' | 'absent'` value, not the raw `RawCongressVote.memberVote` string. The normalized `position` is available in `transformVotes.ts` before `enrichWithCategory` is called (it is derived by `normalizePosition` on the line immediately above).
...
The file header comment (lines 8–10) currently describes the old yea/nay encoding and must be updated to reflect the new `alignedWithIssue` logic.
...
> **Note:** votes where `alignedWithIssue === null` (unmapped or absent) are intentionally excluded from `total`. A category with many votes but few mapped ones will show the `total === 0` fallback.
...
- Fallback renders on initial paint before `useEffect` fires (component renders fallback first, then updates — do not read sessionStorage synchronously during render)
...
All 180 existing tests continue to pass. The following test files contain direct `Vote` object constructions that need `alignedWithIssue` added to their `makeVote` factories:
- `__tests__/voteprint/VoteList.test.tsx` — `makeVote` factory
- `__tests__/voteprint/Voteprint.test.tsx` — `makeVote` factory
- Any other test file constructing a literal `Vote` object

`transformVotes.test.ts` tests call `transformVotes` and check output shape — these will pass only if `transformVotes` correctly includes `alignedWithIssue` in its returned objects (verified by the existing passthrough-fields test).
...
| `lib/voteprint/types.ts` | Add `alignedWithIssue` to `Vote` |
| `lib/voteprint/voteMappings.ts` | Add `stance` to private `VoteMappingEntry`; update `enrichWithCategory` signature and derivation |
| `lib/voteprint/transformVotes.ts` | Pass normalized `position` to `enrichWithCategory`; spread `alignedWithIssue` into Vote |
| `components/Voteprint/Voteprint.tsx` | Straight lines; spoke encoding via `alignedWithIssue`; update file header comment |
...
| `components/RepHeader/RepHeader.tsx` | `'use client'`; SSR fallback; reads sessionStorage on mount; photo, phone, website |
...
| `app/rep/[bioguideId]/page.tsx` | Remove display searchParams; pass minimal props to RepHeader; update file header comment |
...
| `__tests__/…` | New tests per Section 5; update `makeVote` factories with `alignedWithIssue` |
